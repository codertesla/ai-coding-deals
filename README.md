...
- **Free OpenAI-Compatible Inference Endpoint (BYOK):** **NVIDIA NIM Free Tier** — one API key across a large model catalog (incl. popular Chinese models), compatible with the OpenAI SDK via `base_url="https://integrate.api.nvidia.com/v1"`. Best for prototyping and model comparisons (rate-limited; no SLA; terms/logging policies apply). Signup may require phone OTP; rate limits vary (e.g. dashboard may show “up to 40 RPM”). See [build.nvidia.com](https://build.nvidia.com/) and the [model catalog](https://build.nvidia.com/models).
